...
// size of the image
const int64_t SIZE = 16384ULL; //we have enough memory (16GiB)
const int64_t REPEAT = 2ULL; //changed to 2 because less then 20 seconds, original 1ULL
...
int main(int argc, char* argv[]) {
	...
	for (int64_t r = 0; r < REPEAT; ++r) {
		for (int64_t j = 1; j < SIZE - 1; j++) { //for i loop is swapped with for j loop for optimization
...
#else
	std::cout << "max memory:                   " << usage.ru_maxrss << " KiB" << std::endl; //the total amount of physical memory assigned to a process at a given point in time.
#endif
